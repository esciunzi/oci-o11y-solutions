...
def handler(ctx, data: io.BytesIO = None):
    try:
        ...
        dest_namespace='<Object Storage name space>'
        upload_bucket_name = 'Cost_Usage_Reports'  
            ...
            with open(local_file_path, 'rb') as file_content:
                object_storage.put_object(
                    ...
                    bucket_name=upload_bucket_name,
                  	object_name=f"{filename}_{yesterday.year}_{yesterday.strftime('%m')}_{yesterday.strftime('%d')}",
                    print(f"new file name is {object_name}")
